...
function computeRowHeight(containerWidth: number, columnCount: number): number {
  const cardWidth = (containerWidth - (columnCount - 1) * GRID_GAP) / columnCount;
...
export default function AlbumGrid({ albums, horizontal, scrollRef }: AlbumGridProps) {
  ...
  const columnCount = containerWidth > 0 ? computeColumnCount(containerWidth) : 4;
  const estimatedRowHeight = containerWidth > 0 ? computeRowHeight(containerWidth, columnCount) : 300;

  const rows = useMemo(() => chunkArray(albums, columnCount), [albums, columnCount]);
  ...
  const rowVirtualizer = useVirtualizer({
    count: rows.length,
    getScrollElement: () => (horizontal ? null : (scrollRef?.current ?? containerRef.current)),
  ...
  if (horizontal) {
    return (
      <div className={`album-grid horizontal`}>
  ...
  return (
    <div
      ref={containerRef}
      style={{ position: 'relative', height: `${rowVirtualizer.getTotalSize()}px`, padding: '8px 0' }}
    >
        ...
        return (
          ...
          >
            {rowAlbums.map((album, i) => {
              ...
              return (
                <div key={`${album.name}-${album.artist}-${globalIdx}`} style={{ flex: 1, minWidth: 0 }}>
            ...
            {rowAlbums.length < columnCount &&
              Array.from({ length: columnCount - rowAlbums.length }).map((_, i) => (
